...
#define OTP_BASE_SECTORS                    (0x0004FD)
#define SEEPROM_BASE_SECTORS                (0x0004FF)
...
void writeToSD(int nandType, int driveLetter, const char *acFromFilepath)
{
    ...
    if(be64(sign_sector.signature) != NAND_DUMP_SIGNATURE)
    {
        if(nandType == 0) {
            ...
            nand_offset = (uint64_t)SEEPROM_BASE_SECTORS * (uint64_t)BYTES_PER_SECTOR;
        }
        else if(nandType == 4) {
            nand_offset = (uint64_t)OTP_BASE_SECTORS * (uint64_t)BYTES_PER_SECTOR;
        }
    }
    else
    {
        ...
        while(be32(sign_sector.nand_descriptions[nand_idx].nand_type) != 0)
        {
            ...
            else if((nandType == 3) && be32(sign_sector.nand_descriptions[nand_idx].nand_type) == NAND_DESC_TYPE_SEEPROM)
            {
                nand_offset = (uint64_t)be32(sign_sector.nand_descriptions[nand_idx].base_sector) * (uint64_t)BYTES_PER_SECTOR;
                found = 1;
                break;
            }
            else if((nandType == 4) && be32(sign_sector.nand_descriptions[nand_idx].nand_type) == NAND_DESC_TYPE_OTP)
            {
                nand_offset = (uint64_t)be32(sign_sector.nand_descriptions[nand_idx].base_sector) * (uint64_t)BYTES_PER_SECTOR;
                found = 1;
                break;
    ...
    if(error == 0)
    {
        ...
        if(nandType == 0)  {
            ...
            sign_sector.nand_descriptions[nandType].nand_type = be32(NAND_DESC_TYPE_SEEPROM);
        }
        else if(nandType == 4) {
            sign_sector.nand_descriptions[nandType].nand_type = be32(NAND_DESC_TYPE_OTP);
        }
...
static char szClassName[ ] = "sdio nand manager";
...
static const TCHAR nandTypes[5][16] =
{
    TEXT("SLC"),
    TEXT("SLCCMPT"),
    TEXT("MLC"),
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)                  /* handle the messages */
    {
        case WM_CREATE: {
            ...
            combobox = CreateWindow("COMBOBOX", "",  CBS_DROPDOWN | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE,  150, 10, 90, 120, hwnd, 0, 0, 0);
            ...
            SendMessage(combobox,(UINT) CB_ADDSTRING,(WPARAM) 0,(LPARAM)nandTypes[3]);
            SendMessage(combobox,(UINT) CB_ADDSTRING,(WPARAM) 0,(LPARAM)nandTypes[4]);
        ...
        case WM_COMMAND:
            ...
            {
                char acToFilepath[1024];
                memset(acToFilepath, 0, sizeof(acToFilepath));
                int nandType = SendMessage(combobox, (UINT) CB_GETCURSEL, (WPARAM) 0, (LPARAM) 0);
                GetWindowText(editBox2, acToFilepath, sizeof(acToFilepath)-1);
                char *ptr = strrchr(acToFilepath, '\\');
                if(!ptr)
                    ptr = strrchr(acToFilepath, '/');
                if(ptr)
                {
                    const char *cpNandBinName = "";

                    switch(nandType)
                    {
                    case 0:
                        cpNandBinName = "slc.full.img";
                        break;
                    case 1:
                        cpNandBinName = "slccmpt.full.img";
                        break;
                    case 2:
                        cpNandBinName = "mlc.full.img";
                        break;
                    case 3:
                        cpNandBinName = "seeprom.bin";
                        break;
                    case 4:
                        cpNandBinName = "otp.bin";
                        break;
                    }
                    sprintf(ptr+1, "%s", cpNandBinName);
                    SetWindowText(editBox2, acToFilepath);
                }
            }
            break;
        case WM_DESTROY:
